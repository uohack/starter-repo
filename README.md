...
Second, they force you to write about your project in a way that makes you to think critically about your choices. When working on a project, you might take some shortcuts or make decisions that would not be obvious to the causal reader. A README provides a space for you to explain those decisions and forces you to think about them a second time. And sometimes this process helps the writer come up with a better solution than they had in the first place.
...
The best way to learn markdown, after reading [any of the helpful introductions](https://www.google.com/search?q=markdown+introduction), is to start reading and writing it. I promise, you will pick it up quickly.
...
It took me a while to understand the benefits of a .gitignore file, but now I can't live without one. These dot-files are actually hidden by default. Their function is to tell Github (really git, but that's a different conversation) which files to ignore.

### Aside: Hidden files

No matter your system (PC or Mac), your computer has a whole bunch of hidden files that you can choose to display. These are files that your computer does not display by default because some of them are very important and should not be messed with. They typically start with a period, hence dot-files. This tells the system that they should be hidden.

On both a PC and a Mac you can toggle the visibility of hidden files. Just Google your OS and "show hidden files" and it should come right up. You may need to do this to see .gitignore files. Please use caution and don't just edit or trash dot-files on a whim.

### Why would you want to ignore files?

Well, some code processes create logs or cache files that are not necessary to commit to Github. Other times, you'll have secrets files that include passwords or other credentials that you want to keep secret. Maybe you have a bunch of videos or large files and you don't want to put online.

In any of these cases, you can use a .gitignore to ignore those files.

**Be cautious though, if you accidentally commit files you didn't want to, it takes some work to get rid of them. You can't just add them to your .gitignore and hope they disappear from your previous commits. This is not how it works. Github saves all those old commits. Be sure to start your project with a .gitignore and add new files to it as you make them.**

If you do find yourself in that situation (we all have), try to make your repo private (it's good to keep one private repo handy) and Google how to get rid of it. Depending on how far you got in the process determines what you need to do to fix it. And, of course, all of this can be avoided if you use a .gitignore file correctly.
...
def get_secret(service, token='null'):
	...
	# First, we need to figure out where this secrets file is
	# In this case, it's in the same place as my script
	# This might require more configuration on more complex projects
	# Note: This requires os to be imported
...
These are public, so be sure not to include secrets in there.
...
Note: In the wikis tab there is a home wiki and you can create infinite sub-wikis. If that doesn't make sense, don't worry about it, you'll see.
